...
import { errorMessage, isAbortError, requestMenuAnalysis } from "@/lib/ui/api";
import { getDeviceId } from "@/lib/ui/device";
import { STR } from "@/lib/ui/strings";
import { useSettings, type CameraMode } from "@/lib/ui/store";

/*
 * Menü tarama akışı: fotoğraf alma → kırpma → analiz.
 *
 * Etiket akışıyla aynı kamera bileşenlerini kullanır; farkı yönerge
 * metinleri ve gönderilen uçtur (/api/analyze-menu). Kamera ekranı her zaman
 * koyudur (bkz. CLAUDE.md, Bölüm 8).
 */
    ...
    <div className="screen-h flex flex-col bg-[#0b0f0d] text-white">
      ...
      {!mounted ? (
        ...
        <AnalysisProgress
          ...
          steps={STR.menuAnalysisSteps}
        />
      ) : stage === "crop" && file ? (
        <CropEditor
          file={file}
          title={STR.menuCropTitle}
          hint={STR.menuCropHint}
          onCancel={() => {
            setFile(null);
            setStage("capture");
          }}
        ...
        <div className="flex flex-1 flex-col">
          ...
          {cameraMode === "live" ? (
            <LiveCamera
              onCapture={onFile}
              fallback={deviceCameraButton}
              hint={STR.menuLiveCameraHint}
            />
          ) : (
